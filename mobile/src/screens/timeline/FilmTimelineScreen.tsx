/**
 * FilmTimelineScreen — rolls grid shown on the Timeline tab when AppMode is
 * 'film'.
 *
 * Extracted from HomeScreen in C1 so the film-mode hooks (useApiQuery,
 * useFocusEffect, animation refs) always run regardless of which branch
 * HomeScreen picks — the previous early-return-on-mode structure violated the
 * Rules of Hooks (different hook count between 'film' and 'digital' renders).
 *
 * Digital-mode rendering lives in DigitalTimelineScreen; HomeScreen is now a
 * pure conditional dispatcher that renders one or the other.
 */
...
export default function FilmTimelineScreen({ navigation }: any) {
          ...
          <View style={styles.coverWrapper}>
            ...
            <View style={styles.cardContent}>
              <Text style={styles.cardTitle} numberOfLines={1}>
                {item.title || t('home.rollFallback', { id: item.id })}
              </Text>
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      {loading && rolls.length === 0 ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={[styles.emptyTitle, { color: theme.colors.onSurface }]}>{t('home.emptyTitle')}</Text>
        ...
        <>
          ...
          <View style={styles.countBar}>
            <Text style={[styles.countText, { color: theme.colors.onSurfaceVariant }]}>
              {selectedYear ? t('common.rollsInYear', { count: filteredRolls.length, year: selectedYear }) : t('common.rollsCount', { count: filteredRolls.length })}
            </Text>
          </View>
      ...
      <FAB
        ...
        accessibilityLabel={t('home.logShot')}
      />
