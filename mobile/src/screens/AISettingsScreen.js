...
export default function AISettingsScreen() {
  ...
  const loadConfig = useCallback(async () => {
    try {
      const data = await getAIConfig(baseUrl);
      setConfig(data);
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const patch = { api_base_url: apiBaseUrl, text_model: textModel, vision_model: visionModel };
  ...
  const labelStyle = { fontSize: 14, fontWeight: '600', color: theme.colors.primary, marginBottom: 4, marginTop: 16 };
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.colors.background }} contentContainerStyle={{ padding: 20, paddingBottom: 60 }}>
      ...
      {/* API 连接 */}
      <Text style={{ fontSize: 16, fontWeight: 'bold', color: theme.colors.primary, marginBottom: 4 }}>API 连接</Text>
      ...
      {/* 模型选择 */}
      <Text style={{ fontSize: 16, fontWeight: 'bold', color: theme.colors.primary, marginTop: 24, marginBottom: 4 }}>模型选择</Text>
