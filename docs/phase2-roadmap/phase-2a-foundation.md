...
| 2A.3.2 portDiscovery 常量源 | ✅ | server `mdns-service.js`、mobile `portDiscovery.js`、watch `portDiscovery.ts` 三端常量 + 纯工具函数（cleanIpAddress/extractPort/buildUrl/isPrivateIp）全部下沉到 `@filmgallery/shared/portDiscovery`（单一来源，已测）。新增 `portDiscovery.d.ts` 供 watch TS 导入；运行时扫描/mDNS 逻辑各端保留。watch tsc 验证：我的改动 **0 新增错误**（仅余 2 个预先存在的无关错误：react-native-zeroconf 缺 @types、@filmgallery/types 的 `id?: number` 严格性——属 2C/类型收紧范畴） |
| 2A.3.3 api-client | ✅ 硬化 + watch/mobile 采用完成 | api-client 修复 3 个路径 bug、补端点（photos random/geo/negatives、stats、metadata）、**新增韧性**（retry + 主备粘性 failover + 超时 abort + setBaseUrl，对齐原 axios 行为）+ `index.d.ts` + 测试。**watch 已迁移**（services/api.ts 用 createApiClient，retry 2/15s，移除 axios）。**mobile 已迁移**：新增 `src/api/client.js`（Proxy 转发 + `configureApi` 主备 failover + onError→Alert，对齐旧 setupAxios 语义）；3 个 api 模块 + 12 个屏幕/组件的内联调用全部改走 `api.http.*`，移除 `res.data` 适配；删除 `setupAxios.js` + axios 依赖；顺手修复 `getFilmFormats` 路径（/film-formats→/formats）。Proxy 转发模式已 node 验证（含 reconfigure 后生效）；273 tests green。client `core.js` 与 api-client 是合理上下文分叉，保留。**watch/mobile 运行时终验需 metro/tsc 环境** |
...
| 2A.4 mobile TS / workspace | 🟨 进行中（T1 W0 完成 + Layer D 环境打通） | T1 mobile TS 迁移：**W0 工具链 bootstrap 已完成并 commit（`9a8226a`，未 push）**——tsconfig/jest-expo/types/shims/CI mobile job(+metro bundle)/metro.config monorepo 修复/coordTransform.d.ts/zeroconf patch 重生成；四道自动门禁 green（typecheck/jest 4/4/metro 7.8M/root 273）。**Layer D emulator 运行时环境全打通**：Android SDK 全组件（NDK r27d/build-tools 35/platform-36/cmake 软链/system-image 31）+ AVD test31 + app 编译 186M APK 装机启动 + JS bundle 经 metro+adb reverse 加载 + 连通测试 server 渲染真实数据。详见 `phase-2a-mobile-ts-migration.md`。**剩余**：W1–W8 源码 TS 迁移（8 个 Wave，66 文件）；T2 workspace 升级非必需 |
