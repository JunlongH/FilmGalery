...
| 14 | FilmLab.jsx:1364, 1671, 1762, 2195 + CpuRenderService.js:170 | **`RenderCore` 每次渲染都 `new` + `prepareLUTs()`**。5 处创建点（v3 修正：原计划漏列 CpuRenderService.js:170；行号 1647/1738/2179 应为 1671/1762/2195）。LUT 无跨实例缓存 | 复用实例（ref + params 变化时重建）；或模块级 LUT 缓存 |
...
| 17 | PhotoSwitcher.jsx:24 | **PhotoThumb 未 memo**，父组件 PhotoSwitcher 在 `currentParams` 变化时重渲染（v3 修正：原诊断称 PhotoThumb 接收 `currentParams` prop 不准——实际 prop 列表为 `{photo, isActive, onClick, hasPositive, isSelected, showCheckbox}`，但无 memo 时父组件重渲染会连带 36+ 子组件重渲染） | `React.memo` + 稳定 `onClick`（`useCallback`） |
| 23 | ComputeService.js:158-166,261,364,485 + FilmLab.jsx:1367-1402 + AutoCropButton.jsx:49-109 | **全管线零 `AbortController`**（v3 修正：由 P2 升级 P1——切换照片时旧 fetch/XHR/CPU 渲染继续飞行，旧响应覆盖新状态，是正确性问题而非纯性能）。6 处 fetch + XHR + CPU 渲染 + 边缘检测 | 接受 `AbortSignal`；CPU 渲染用 `shouldAbort` 回调；调用方切换时 `abortRef.current?.abort()` |
| 28 | FilmLab.jsx:224-230 | **AI 上下文 `updateOverlayContext` 每滑块触发**（v3 修正：由 P2 升级 P1——直接 UX 影响，每滑块拖动都触发 AI 面板上下文更新+重渲染） | debounce 300ms |
| 53 | FilmLabWebGL.js:74-81 vs 88 | **`isWebGLAvailable()` 只查 `webgl`/`experimental-webgl`，但 `processImageWebGL()` 优先 `webgl2`**（v3 新发现）。两者判定不一致：若浏览器支持 `webgl2` 但对 `webgl` 返回 null，GPU 路径被误禁用 | `isWebGLAvailable()` 改为优先 `getContext('webgl2')`，与 `processImageWebGL` 对齐 |

## P2 — 中等影响（20 项）

| # | 文件:行号 | 问题 | 建议 |
|---|---|---|---|
| 7 | FilmLab.jsx:1358 vs 1416 vs 150 | **直方图属性名不一致**（v3 修正：由 P1 降级 P2——仅 error path 触发，cosmetic 影响）。错误路径用 `r/g/b`，正常路径用 `red/green/blue`。错误路径下 ToneCurveEditor 直方图为空 | 统一为 `{ rgb, red, green, blue }`，删除 `maxCount` |
| 18 | FilmLab.jsx:1299-1302 | **`processImage` 每帧分配 4× `new Array(256)`**（v3 修正：由 P1 降级 P2——8KB/帧 vs P0-3 的 12MB/帧，可忽略）。直方图缓冲 | `useRef(new Array(256).fill(0))` 复用 |
...
| 35 | CpuRenderService.js:148-152 | **`processCanvasWithRenderCore`（sync）阻塞主线程** | 弃用或重命名 `-Blocking`，默认 async |
| 36 | FilmLabControls.jsx:949 | **内联 `require()` 在 JSX 回调** | 顶部静态 import |
| 54 | FilmLab.jsx:1765-1777（v3 新发现） | **`handleSave` 同步像素循环阻塞主线程**（同 P0-2 模式但触发于 Save）。6000×4000 = 24M 像素，秒级阻塞 | 复用 `processCanvasWithRenderCoreAsync` |
| 55 | FilmLab.jsx:2198-2210（v3 新发现） | **`downloadClientJPEG` 同步像素循环阻塞**（同 #54）。P2-34 抽公共函数后仍同步阻塞 | 抽公共 async 函数 |
...
| 56 | FilmLabWebGL.js:74-81（v3 新发现） | **`isWebGLAvailable()` 每次调用创建 canvas**（00-overview.md:77 提到但原 P3 列表遗漏）— 与 #53 配套修复时一并解决 | 模块级 `let cached` 缓存结果 |
| 57 | FilmLab.jsx:1181（v3 新发现） | **`webglParams` 引用相等性脆弱**：`lastWebglParamsRef.current === webglParams` 用 `===`，若 memo 依赖中任一对象（`curves`/`hslParams`/`splitToning`/`densityLevels`/`committedCrop`）拿到新引用但值相同，memo 重算 → 缓存未命中 → 触发 P0-1 canvas 重建 | 深比较（`stableSerializeParams`）或对依赖做值比较 |
| 58 | FilmLab.jsx:1418-1421（v3 新发现） | **`processImage` 错误静默吞掉**：`catch (processImageError) { console.error(...) }` 仅 log，canvas 显示陈旧内容无任何用户反馈。WebGL context loss 后可能反复失败 | 加 retry 计数 + error state（UI 提示） |
...
| P1 | 15 | React 重渲染、状态管理、竞态、每像素开销、AbortController、AI 上下文、webgl2 检测 |
| P2 | 20 | 内存分配、代码重复、调试代码、Save/Export 同步阻塞、histogram 命名 |
| P3 | 18 | 死代码、console.log、内联箭头、minor 分配、canvas 创建、引用相等、错误吞掉 |

> **v3 修正说明（2026-07-21 review 后）**：
> - 行号修正：P1-14 的 1647/1738/2179 应为 1671/1762/2195；新增遗漏的第 5 处 `CpuRenderService.js:170`。
> - 优先级调整：P2-23（AbortController）→ P1（正确性问题）；P2-28（AI 上下文）→ P1（直接 UX 影响）；P1-7（histogram 命名）→ P2（cosmetic、仅 error path）；P1-18（4× Array(256)）→ P2（8KB/帧可忽略）。
