...
| 2A.3.3 api-client | 🟨 硬化+watch 采用完成 / mobile 待迁移 | api-client 修复 3 个路径 bug（photos.get→/single/:id、getFavorites→/favorites、locations.search→/api/locations?q=）、补端点（photos random/geo/negatives、stats、metadata 模块）、**新增韧性**（同址 retry + 主备粘性 failover + 超时 abort + setBaseUrl，对齐原 axios 行为）+ `index.d.ts` + 13 个新测试（**273 tests green**）。client `core.js` 与 api-client 是合理上下文分叉，保留。**watch 已迁移**：`services/api.ts` 改用 `createApiClient`（retry 2 次/15s 超时），保留 8 方法签名 + films 缓存 + envelope + shot_logs 编码，移除 axios 依赖。**mobile 待迁移**（~23 处分散调用 + 主备 failover，跨 13 屏，需 metro 终验——api-client 已就绪，专项推进） |
